  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <header className="py-12 text-center bg-blue-50">
        ...
        <p className="mt-2 text-gray-600">Le meilleur du marché directement chez vous</p>
      </header>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {products.map((prod) => (
              <div key={prod._id} className="bg-white rounded-lg shadow-md overflow-hidden border">
                <img src={prod.imageUrl || '/file.svg'} alt={prod.name} className="h-48 w-full object-cover" />
                <div className="p-4 space-y-2">
                  <h3 className="font-bold text-lg">{prod.name}</h3>
                  <p className="text-sm text-gray-600">{prod.description}</p>
                  <div className="flex justify-between items-center pt-2">
                    <span className="text-xl font-bold text-blue-600">{prod.price} FCFA</span>
